...
sql = "select * from JWF1312B_bingtiao"
cursor.execute(sql)
db.commit()
results = cursor.fetchall()

tables_join = ['b2s1','b2s2','b2s3','b2s4','b2s5','b2s6']
...
for table_join in tables_join:
	for result,num in zip(results,range(len(results))): 
		...
		values = []
		sql = "select record_id_shumian from {} where record_id_bingtiao = {}".format(table_join, result[0])
		cursor.execute(sql)
		db.commit()
