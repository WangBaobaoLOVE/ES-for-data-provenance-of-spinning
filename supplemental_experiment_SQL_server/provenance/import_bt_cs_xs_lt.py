...
data_bingtiao = pd.read_csv('../../provenance/data/JWF1312B_bingtiao.csv')
...
for row in range(data_bingtiao.shape[0]):
	...
	for i in range(data_bingtiao.shape[1]):
		if i == 1:
		    values = values + '\'' + str(data_bingtiao.iloc[row,i]).split('.')[0] + '\''+ ','
		else: 
...
data_cusha = pd.read_csv('../../provenance/data/JWF1418_cusha.csv')
...
for row in range(data_cusha.shape[0]):
	...
	for i in range(data_cusha.shape[1]):
		if i == 1:
		    values = values + '\'' + str(data_cusha.iloc[row,i]).split('.')[0] + '\''+ ','
		else: 
...
data_xisha = pd.read_csv('../../provenance/data/JWF1562_xisha.csv')
...
for row in range(data_xisha.shape[0]):
	...
	for i in range(data_xisha.shape[1]):
		if i == 0:
		    values = values + '\'' + str(data_xisha.iloc[row,i]).split('.')[0] + '\''+ ','
		else: 
...
data_luotong = pd.read_csv('../../provenance/data/SMARO-E_luotong.csv')
...
for row in range(data_luotong.shape[0]):
	...
	for i in range(data_luotong.shape[1]):
		if i == 0:
		    values = values + '\'' + str(data_luotong.iloc[row,i]).split('.')[0] + '\''+ ','
		else: 
